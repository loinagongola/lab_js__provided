...
function recalculate() {
    ...
    if (modelName.innerHTML === "Model XYZ") {
        let costLabel = parseInt(duration.innerHTML) * 100;
        newTotalCost.innerHTML = costLabel.toString();
    } else if (modelName.innerHTML === "Model CPRG") {
        let costLabel = parseInt(duration.innerHTML) * 213;
        newTotalCost.innerHTML = costLabel.toString();
    }
}
...
function changeDuration() {
  let duration = document.getElementById('duration-text');
    duration.innerHTML = prompt('Enter new Duration: ');
    recalculate();
}
